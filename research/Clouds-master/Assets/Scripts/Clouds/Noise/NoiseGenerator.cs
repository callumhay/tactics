...
public class NoiseGenerator : MonoBehaviour {
    ...
    public void UpdateNoise () {
        ...
        if (updateNoise && noiseCompute) {
            ...
            int worleyKernel = noiseCompute.FindKernel("CSWorley");
            ...
            noiseCompute.SetTexture (worleyKernel, "Result", ActiveTexture);
            ...
            noiseCompute.SetBuffer (worleyKernel, "minMax", minMaxBuffer);
            noiseCompute.Dispatch (worleyKernel, numThreadGroups, numThreadGroups, numThreadGroups);

            // Set normalization kernel data:
            var minMax = new int[2];
            minMaxBuffer.GetData (minMax);
            
            noiseCompute.SetBuffer (normalizeKernel, "minMax", minMaxBuffer);
            noiseCompute.SetTexture (normalizeKernel, "Result", ActiveTexture);
            // Dispatch normalization kernel
            noiseCompute.Dispatch (normalizeKernel, numThreadGroups, numThreadGroups, numThreadGroups);
            ...
            if (logComputeTime) {
                ...
                minMaxBuffer.GetData (minMax);
                Debug.Log("MIN: " + minMax[0] + ", MAX: " + minMax[1]);

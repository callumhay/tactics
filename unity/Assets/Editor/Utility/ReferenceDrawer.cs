...
public class ReferenceDrawer : PropertyDrawer {
  ...
  public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
    ...
    //buttonRect.xMin = position.xMax - 120;
    //position.xMax = 
  ...
  public ScriptableObject CreateNew(SerializedProperty property) {
    ...
    dest = EditorUtility.SaveFilePanel("Save object as", dest, "New " + type.FullName, "asset");

    // Get the relative path to the asset from the assets directory
    var appDataPath = Path.GetDirectoryName(Application.dataPath);
    dest = PathHelper.GetRelativePath(dest, appDataPath);

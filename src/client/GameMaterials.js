...
const MATERIAL_TYPE_WATER   = "water";
...
const materials = {
  ...
  [MATERIAL_TYPE_WATER]: {
    type: MATERIAL_TYPE_WATER,
    gameType: GameTypes.WATER,
    density: 1000,
    stiffness: 1e11,
    friction: 0.06,
    restitution: 0,
    three: new THREE.MeshPhongMaterial({color:0x0000ff, shininess:128}),
    cannon: new CANNON.Material(MATERIAL_TYPE_WATER),
  },
  [MATERIAL_TYPE_ROCK]: {
...
const initContactMaterials = (materials) => {
  ...
  for (let i = 0; i < materialValues.length; i++) {
    ...
    for (let j = i; j < materialValues.length; j++) {
      ...
      const contactMat = new CANNON.ContactMaterial(cMat1, cMat2, {
      ...
      assert(contactMat.friction >= 0 && contactMat.friction <= 1);
      assert(contactMat.restitution >= 0 && contactMat.restitution <= 1);
      assert(contactMat.contactEquationStiffness);
      assert(contactMat.frictionEquationStiffness);
      contactMaterials.push(contactMat);
    }
  }
}
...
// NOTE: YOU NEED TO USE LOCAL POSITIONS FOR DEBRIS AND WORLD POSITIONS FOR TERRAIN!!!

...
wsPosition = worldPosition.xyz/worldPosition.w;
...
const dynamicTextureLookupFragCode = `
  {
    ...
    vec2 zyCoord = wsPosition.yz;
    vec2 xzCoord = wsPosition.xz;
    vec2 xyCoord = wsPosition.xy;

    vec4 zyColour = mapTexelToLinear(texture2D(mapZY, zyCoord));
    vec4 xzColour = mapTexelToLinear(texture2D(mapXZ, xzCoord));
    vec4 xyColour = mapTexelToLinear(texture2D(mapXY, xyCoord));
